...
def register():
    ...
    if request.method == "POST":
        ...
        # Construct the filename with current date, last name, and birthdate
...
def training():
    session['idx_train'] = 0
    idx_train = session['idx_train']
    return render_template('training.html', audio_file=sound_files[tr_index[idx_train]], next_index=idx_train)


@app.route('/play_training')
def play_training():
    idx_train = session['idx_train'] + 1
    if idx_train >= len(tr_index):
        idx_train = 0  # Reset the training index if it goes out of bounds
    session['idx_train'] = idx_train
    return render_template('training.html', audio_file=sound_files[tr_index[idx_train]], next_index=idx_train)
